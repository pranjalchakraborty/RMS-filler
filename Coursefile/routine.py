...
def generate_fill_instructions(model, routine1_context, routine2_context, clean_template_context, teacher_initials):
    """
    Provides the API with full context and asks it to return a JSON array
    of instructions for filling the clean routine for a specific teacher.
    """
    prompt = f"""
    You are an expert system for academic schedule consolidation. Your task is to analyze two source routines and generate a set of precise instructions to fill a clean template for a teacher with the initials "{teacher_initials}".
    ...
    **YOUR TASK:**
    ...
    2.  **Find Teacher's Classes:** Analyze the 'cells' list in the blueprints for Routine 1 and Routine 2. Find all cell objects where the `text` contains the marker "{teacher_initials}" (e.g., "({teacher_initials})", "{teacher_initials}/", " {teacher_initials} ").
    3.  **Generate Instructions:** For each class you find for "{teacher_initials}", you must generate one or more fill instructions.
    ...
    4.  **Handle Conflicts:** If two different classes for "{teacher_initials}" are scheduled for the exact same coordinate `(r, c)`, combine their text into a single instruction for that coordinate, separated by a newline (`\\n`).
    ...
    """
    logging.info(f"--- PROMPT SENT TO GEMINI API for teacher: {teacher_initials} ---")
...
def main():
    ...
    fill_instructions_json_str = generate_fill_instructions(gemini_model, routine1_context, routine2_context, clean_template_context, teacher_initials)
